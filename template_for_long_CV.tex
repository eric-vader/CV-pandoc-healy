...
%
% \textit{$appointment.items.item$}\hfill\sdate\today\,-\,\sydate\today\newline\justifying{\small $appointment.items.description$}\\
$for(appointment)$
\subsection{\textbf{$appointment.place$}}
\begin{separator}
  $for(appointment.items/allbutlast)$
    \entry{\today}{\today}{\textit{$appointment.items.item$}
